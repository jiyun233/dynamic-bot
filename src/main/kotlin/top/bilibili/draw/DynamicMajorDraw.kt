    ...
    return try {
        createImage(
            ...
            if (badge.text.isNotBlank()) {
                ...
                val labelTextLine = TextLine.make(badge.text, labelFont)
                try {
                    canvas.drawLabelCard(
                        labelTextLine,
                        commonCardRect.right - labelTextLine.width - quality.badgePadding * 4 - quality.cardPadding,
...
suspend fun drawPgcCard(
    ...
    return try {
        ...
        coverImg.close()
        titleParagraph.close()
        broadcastParagraph.close()
        statusParagraph.close()
        statParagraph.close()
        evaluateParagraph?.close()
    }
}
...
suspend fun drawLiveSmallCard(
    ...
    return try {
        ...
        if (duration != null) {
            val durationFont = font.makeWithSize(quality.subTitleFontSize)
            val durationTextLine = TextLine.make(duration, durationFont)
            try {
                canvas.drawLabelCard(
                    durationTextLine,
                    coverRRect.left + quality.badgePadding * 2,
...
suspend fun ModuleDynamic.Major.Article.drawGeneral(): Image {
    ...
    return createImage(
        ...
        if (BiliConfigManager.config.imageConfig.badgeEnable.left) {
            ...
            val labelFont = font.makeWithSize(quality.subTitleFontSize)
            val labelTextLine = TextLine.make("专栏", labelFont)
            try {
                canvas.drawLabelCard(
                    labelTextLine,
                    articleCardRect.right - labelTextLine.width - quality.badgePadding * 4 - quality.cardPadding,
